    ...
    public void Validate()
    {
        if (Enabled)
        {
            if (string.IsNullOrWhiteSpace(PushUrl))
                throw new InvalidOperationException("HeartbeatConfig: When enabled, PushUrl is required and cannot be empty. Check PANDABOT_Bot__Heartbeat__PushUrl environment variable.");
