...
public sealed class VersionManagerBehaviorTests
{
    ...
    {
        var repoRoot = FindRepoRoot("PandaBot.sln");
        BuildVersionManager(repoRoot);

        var tempDir = Path.Combine(Path.GetTempPath(), "panda-vm-test-" + Guid.NewGuid().ToString("N"));
    ...
    private static string BuildChangelog(string unreleasedBullet)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("All notable changes to PandaBot will be documented in this file.");
        sb.AppendLine();
