...
public sealed class SingleMessageServiceTests : IAsyncDisposable
{
    ...
    public SingleMessageServiceTests()
    {
        ...
        services.AddDbContext<PandaBotContext>(o =>
    ...
    private async Task<T> UseDbAsync<T>(Func<PandaBotContext, Task<T>> action)
    {
    ...
    private async Task UseDbAsync(Func<PandaBotContext, Task> action)
    {
        await using var scope = _scopeFactory.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<PandaBotContext>();
        await action(db);
